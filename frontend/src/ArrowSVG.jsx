...
export default function ArrowSVG({ onPartClick, activePart, onClearSelection, mode = 'arrow' }) {
  ...
  if (mode === 'bolt') {
    ...
    return (
      <div className="relative mb-0 md:mb-10 flex justify-center md:justify-center w-full h-full">
        <div className="md:w-[950px] w-full h-full flex justify-center md:justify-center items-stretch md:items-center">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox={`0 0 ${BOLT_W} ${BOLT_H}`}
            className="h-full w-full md:w-[950px] md:h-auto transform md:transform-none rotate-90 md:rotate-0 origin-center md:origin-center transition-transform duration-300 scale-[2.9] md:scale-100"
            // Previous mobile scale: scale-[2.6] (changed 2024-12-19, increased to 2.9 on 2024-12-19)
          ...
          {/* Left side group (knock, fletching, shaft) - moves left on insert hover or active */}
          <g className={`transition-transform duration-200 transform-gpu ${(hoverPart === 'insert' || activePart === 'insert') ? '-translate-x-3' : ''}`}>
          ...
          {/* Insert - moves right on shaft hover or active (more aggressive for bolt) */}
          <rect
            className={`${hot('insert') ? 'fill-blaze' : 'fill-gray-700 dark:fill-gray-300'} cursor-pointer transition-transform duration-200 hover:fill-blaze ${(hoverPart === 'shaft' || activePart === 'shaft') ? 'translate-x-2' : ''}`}
          ...
          >
            <path
              className={`transition-colors duration-200 ${hot('tip') ? 'fill-blaze' : 'fill-gray-700 dark:fill-gray-300'}`}
  ...
  return (
    <div className="relative mb-0 md:mb-10 flex justify-center md:justify-center w-full h-full">
      <div className="md:w-[950px] w-full h-full flex justify-center md:justify-center items-stretch md:items-center">
        ...
        {/* Insert - moves right on shaft hover or active */}
        <rect
          className={`${hot('insert') ? 'fill-blaze' : 'fill-gray-700 dark:fill-gray-300'} cursor-pointer transition-transform duration-200 hover:fill-blaze ${(hoverPart === 'shaft' || activePart === 'shaft') ? 'translate-x-1' : ''}`}
